...
class Event
{
  ...
  */ 
  ...
  /** 
  * Change the description of a ticket type for an event 
  * @param string Event id
  * @param string Ticket type id
  * @param string The ticket type description
  * @return string 
  * @key admin __exec
  */ 
  static function set_ttype_description($id, $ttype, $value)
  {
    if(!$event = MongoLib::findOne_editable('events', $id))
      return ErrorLib::set_error("That event is not within your domain");
    ...
    // make change
    $event['ttypes'][$ttype['key']]['description'] = $value;
    $update['ttypes'] = $event['ttypes'];
    MongoLib::set('events', $id, $update);

    History::add('events', $id, array('action' => 'set_ttype_description', 'value' => $event['ttypes'][$ttype['key']]));
  ...
  */ 
  static function replicate($id)
  {
    ...
    if($event['name']) 
      self::set_name($new_id, $event['name']);
    ...
    if($event['key']) 
      self::set_key($new_id, $event['key'] . '-' . substr(sha1(mt_rand()), 0, 3));
